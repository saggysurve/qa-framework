...
async function exec(cmd, args = [], originalOpts = {}) {
    ...
    const defaults = {
        ...
        shell: undefined,
        logger: undefined,
        maxStdoutBufferSize: circular_buffer_js_1.MAX_BUFFER_SIZE,
        maxStderrBufferSize: circular_buffer_js_1.MAX_BUFFER_SIZE,
    };
    ...
    return await new Promise((resolve, reject) => {
        ...
        const stdoutBuffer = new circular_buffer_js_1.CircularBuffer(opts.maxStdoutBufferSize);
        const stderrBuffer = new circular_buffer_js_1.CircularBuffer(opts.maxStderrBufferSize);
        let timer = null;
        proc.on('error', async (err) => {
            let error = err;
            if (error.code === 'ENOENT') {
                error = await (0, helpers_js_1.formatEnoent)(error, cmd, opts.cwd?.toString());
            }
            reject(error);
        });
        if (proc.stdin) {
