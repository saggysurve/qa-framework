...
class SubProcess extends node_events_1.EventEmitter {
    ...
    async start(startDetector = null, timeoutMs = null, detach = false) {
        ...
        if ((typeof startDetector === 'boolean' || startDetector instanceof Boolean) && Boolean(startDetector)) {
        ...
        else if ((typeof timeoutMs === 'boolean' || timeoutMs instanceof Boolean) && Boolean(timeoutMs)) {
            if (!this.opts.detached) {
                throw new Error(`Unable to detach process that is not started with 'detached' option`);
            }
            detach = true;
        ...
        return await new Promise((resolve, reject) => {
            ...
            this.proc.on('error', async (err) => {
                ...
                if (error.code === 'ENOENT') {
                    error = await (0, helpers_js_1.formatEnoent)(error, this.cmd, this.opts?.cwd?.toString());
                }
                reject(error);
                this.proc?.unref();
                this.proc = null;
