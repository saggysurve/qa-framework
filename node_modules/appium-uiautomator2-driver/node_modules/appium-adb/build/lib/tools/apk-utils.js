...
import { APKS_EXTENSION, APK_INSTALL_TIMEOUT, DEFAULT_ADB_EXEC_TIMEOUT, buildInstallArgs, cloneDeep, defaults, readPackageManifest, } from '../utils/index.js';
...
export async function getApplicationInstallState(appPath, pkg = null) {
    ...
    if (Number.isInteger(apkVersionCode) && Number.isInteger(pkgVersionCode)) {
        ...
        if (pkgVersionCode === apkVersionCode) {
            if (typeof apkVersionName === 'string' &&
    ...
    else if (typeof apkVersionName === 'string' &&
        typeof pkgVersionName === 'string' &&
        semver.satisfies(pkgVersionName, `>=${apkVersionName}`)) {
        log.debug(`The version name of the installed '${pkg}' is greater or equal to the application version name ('${pkgVersionName}' >= '${apkVersionName}')`);
        return semver.satisfies(pkgVersionName, `>${apkVersionName}`)
 ...
 * will be undefined.
 */
export async function extractStringsFromApk(appPath, language = null, outRoot = null) {
    ...
    try {
        ...
        configMarker = await formatConfigMarker(async () => {
            const { stdout } = await exec(this.binaries.aapt, ['d', 'configurations', appPath]);
        ...
        configMarker = await formatConfigMarker(async () => {
            const { stdout } = await exec(this.binaries.aapt2, ['d', 'configurations', appPath]);
            return util.uniq(stdout.split(os.EOL));
        }, language, '');
        try {
            const { stdout } = await exec(this.binaries.aapt2, ['d', 'resources', appPath]);
    ...
    if (util.isEmpty(apkStrings)) {
        ...
        log.info(`Successfully extracted ${Object.keys(apkStrings).length} strings from ` +
