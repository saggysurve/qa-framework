...
export async function activateApp(appId) {
    ...
    if (activityName === RESOLVER_ACTIVITY_NAME) {
        ...
        log.debug(`The launchable activity name of '${appId}' was resolved to '${activityName}'. ` +
 ...
 *
 */
export async function listInstalledPackages(opts = {}) {
    const { user } = opts;
    ...
    try {
        ...
        if (stdout.includes('Error: Activity class') && stdout.includes('does not exist')) {
            if (options.retry && options.activity && !options.activity.startsWith('.')) {
                log.debug(`We tried to start an activity that doesn't exist, retrying with '.${options.activity}' activity name`);
...
export async function getFocusedPackageAndActivity() {
    ...
    if (focusedAppCandidates.length > 1 && currentFocusAppCandidates.length > 0) {
        ...
        return (intersectionWith(focusedAppCandidates, currentFocusAppCandidates, (value, other) => {
...
export async function waitForActivityOrNot(pkg, activity, waitForStop, waitMs = 20000) {
    ...
    log.debug(`Expected package names to ${waitForStop ? 'not ' : ''}be focused within ${waitMs}ms: ` +
    ...
    log.debug(`Expected activity name patterns to ${waitForStop ? 'not ' : ''}be focused within ${waitMs}ms: ` +
    ...
    const conditionFunc = async () => {
        ...
        if (appActivity && appPackage) {
            ...
            const isFound = allPackages.includes(appPackage) && possibleActivityPatterns.some((p) => p.test(fullyQualifiedActivity));
 ...
 */
export function assertSafeComponentName(value, kind) {
    if (!SAFE_COMPONENT_NAME_PATTERN.test(value)) {
        throw new Error(`The ${kind} '${value}' contains illegal characters. Only letters, digits and the ` +
...
export function extractMatchingPermissions(dumpsysOutput, groupNames, grantedState = null) {
    const groupPatternByName = (groupName) => new RegExp(`^(\\s*${util.escapeRegExp(groupName)} permissions:[\\s\\S]+)`, 'm');
