 ...
 */
export const resetTelnetAuthToken = memoize(async function resetTelnetAuthToken() {
...
export async function launchAVD(avdName, opts = {}) {
    ...
    log.debug(`Launching Emulator with AVD ${avdName}, launchTimeout ${launchTimeout}ms and readyTimeout ${readyTimeout}ms`);
    ...
    if (!util.isEmpty(args)) {
        launchArgs.push(...(Array.isArray(args)
            ? args
            : util.shellParse(`${args}`).filter((token) => typeof token === 'string')));
    }
 ...
 */
export const getVersion = memoize(async function getVersion() {
    let stdout;
    try {
    ...
    try {
        await waitForCondition(async () => {
            if (isFirstCheck) {
                ...
                log.debug(`${timer.getDuration().asMilliSeconds.toFixed(0)}ms elapsed since ` +
            ...
            try {
                if (!isBootCompleted) {
                    ...
                    if (bootCompleted.trim() !== '1' || !['stopped', ''].includes(bootAnimState.trim())) {
                        log.debug(`Current status: sys.boot_completed=${bootCompleted.trim()}, ` +
 ...
 */
export const getBuildToolsDirs = memoize(async function getBuildToolsDirs(sdkRoot) {
    ...
    try {
        ...
        const pairs = await asyncmap(buildToolsDirs, async (dir) => [(await fs.stat(dir)).mtime.valueOf(), dir]);
