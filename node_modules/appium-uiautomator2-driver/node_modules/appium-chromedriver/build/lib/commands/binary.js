 ...
 */
export async function getChromedrivers(mapping) {
 ...
 */
export async function updateDriversMapping(newMapping) {
 ...
 */
export async function getCompatibleChromedriver() {
    ...
    throw new Error(`Exceeded ${GET_COMPATIBLE_CHROMEDRIVER_MAX_ITERATIONS} iterations while selecting a compatible Chromedriver.`);
}
/**
 * Resolves and verifies the effective Chromedriver executable path.
 */
export async function initChromedriverPath() {
...
async function mergeDiscoveredMappingGaps(ctx, cds, mapping) {
    ...
    ctx.log.info(`Found ${util.pluralize('Chromedriver', missingCount, true)}, ` +
...
function logChosenMatchingChromedriver(ctx, matchingDrivers, chromeVersion) {
    ...
    ctx.log.debug(`Found ${util.pluralize('executable', matchingDrivers.length, true)} ` +
