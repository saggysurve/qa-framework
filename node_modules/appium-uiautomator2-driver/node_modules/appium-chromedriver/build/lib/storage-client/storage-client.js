...
export class ChromedriverStorageClient {
    ...
    async syncDrivers(opts = {}) {
        ...
        log.debug(`Got ${util.pluralize('driver', driversToSync.length, true)} to sync: ` + JSON.stringify(driversToSync, null, 2));
        const synchronizedDrivers = [];
        const archivesRoot = await tempDir.openDir();
        try {
            await asyncmap([...driversToSync.entries()], async ([idx, driverKey]) => {
    ...
    async getLatestKnownGoodVersion() {
        let jsonStr;
        try {
            jsonStr = await retrieveData(CHROME_FOR_TESTING_LAST_GOOD_VERSIONS, {
                'user-agent': USER_AGENT,
                accept: `application/json, */*`,
    ...
    selectMatchingDrivers(osInfo, opts = {}) {
        ...
        if (!util.isEmpty(driversToSync)) {
            ...
            for (const majorVersion of Object.keys(patchesMap)) {
                ...
                patchesMap[majorVersion].sort((a, b) => compareVersions(b, a));
            }
    ...
    async unzipDriver(src, dst) {
        ...
        try {
            ...
            const chromedriverPath = await fs.walkDir(tmpRoot, true, (itemPath, isDirectory) => !isDirectory && path.parse(itemPath).name.toLowerCase() === 'chromedriver');
            if (!chromedriverPath) {
