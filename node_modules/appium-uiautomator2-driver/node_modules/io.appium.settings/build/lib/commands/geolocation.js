...
export async function setGeoLocation(location, isEmulator = false) {
    ...
    if (isEmulator) {
        ...
        const args = ['am', 'start-foreground-service', '-e', 'longitude', longitude, '-e', 'latitude', latitude];
        if (altitude != null) {
...
export async function getGeoLocation() {
    const output = await this.checkBroadcast(['-n', LOCATION_RECEIVER, '-a', LOCATION_RETRIEVAL_ACTION], 'retrieve geolocation', true);
...
export async function refreshGeoLocationCache(timeoutMs = 20000) {
    ...
    await this.checkBroadcast(['-n', LOCATION_RECEIVER, '-a', LOCATION_RETRIEVAL_ACTION, '--ez', 'forceUpdate', 'true'], 'refresh GPS cache', false);
    if (logcatMonitor && monitoringPromise) {
        ...
        try {
            ...
            this.log.info(LOG_PREFIX, `The GPS cache has been successfully refreshed after ` + `${(performance.now() - startMs).toFixed(0)}ms`);
        }
        finally {
