...
import {NOTIFICATIONS_RETRIEVAL_ACTION, SETTING_NOTIFICATIONS_LISTENER_SERVICE} from '../constants.js';
...
export async function getNotifications(this: SettingsApp): Promise<Record<string, any>> {
  ...
  const output = await this.checkBroadcast(['-a', NOTIFICATIONS_RETRIEVAL_ACTION], 'retrieve notifications');
...
export async function adjustNotificationsPermissions(this: SettingsApp): Promise<boolean> {
  if ((await this.adb.getApiLevel()) >= 29) {
    await this.adb.shell(['cmd', 'notification', 'allow_listener', SETTING_NOTIFICATIONS_LISTENER_SERVICE]);
    return true;
  }
  return false;
}
