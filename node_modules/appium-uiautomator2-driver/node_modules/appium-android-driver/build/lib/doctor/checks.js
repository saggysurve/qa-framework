...
class EnvVarAndPathCheck {
    ...
    async fix() {
        return (`Make sure the environment variable ${console.styleText('bold', this.varName)} is properly configured for the Appium process. ` +
...
export class JavaHomeValueCheck {
    ...
    async diagnose() {
        ...
        if (!(await fs.exists(javaBinary))) {
            return doctor.nok(`${JAVA_HOME_VAR_NAME} is set to an invalid value. ` +
...
export class AndroidSdkCheck {
    ...
    async fix() {
        return (`Manually install ${console.styleText('bold', 'Android SDK')} and set ${console.styleText('bold', 'ANDROID_HOME')}. ` +
...
export class OptionalBundletoolCheck {
    ...
    async fix() {
        return (`${console.styleText('bold', 'bundletool.jar')} is used to handle Android App bundles. ` +
...
export class OptionalGstreamerCheck {
    ...
    async diagnose() {
        ...
        return gstreamerPath && gstInspectPath
            ? doctor.okOptional(`${this.GSTREAMER_BINARY} and ${this.GST_INSPECT_BINARY} are installed at: ${gstreamerPath} and ${gstInspectPath}`)
            : doctor.nokOptional(`${this.GSTREAMER_BINARY} and/or ${this.GST_INSPECT_BINARY} cannot be found`);
    }
    async fix() {
        return (`${`${console.styleText('bold', this.GSTREAMER_BINARY)} and ${console.styleText('bold', this.GST_INSPECT_BINARY)}`} are used to stream the screen of the device under test. ` +
...
export class OptionalFfmpegCheck {
    ...
    async fix() {
        return (`${console.styleText('bold', this.FFMPEG_BINARY)} is used to capture screen recordings from the device under test. ` +
