 ...
 */
export async function mobileChangePermissions(permissions, appPackage, action, target = PERMISSION_TARGET.PM) {
    ...
    switch (target?.toLowerCase()) {
        case PERMISSION_TARGET.PM:
            return await changePermissionsViaPm.bind(this)(permissions, appPackage, action?.toLowerCase());
        case PERMISSION_TARGET.APPOPS:
            this.assertFeatureEnabled(ADB_SHELL_FEATURE);
            return await changePermissionsViaAppops.bind(this)(permissions, appPackage, action?.toLowerCase());
        default:
...
async function changePermissionsViaPm(permissions, appPackage, action) {
    if (!Object.values(PM_ACTION).includes(action)) {
        throw new errors.InvalidArgumentError(`Unknown action '${action}'. Only ${JSON.stringify(Object.values(PM_ACTION))} actions are supported`);
    }
...
async function changePermissionsViaAppops(permissions, appPackage, action) {
    if (!Object.values(APPOPS_ACTION).includes(action)) {
        throw new errors.InvalidArgumentError(`Unknown action '${action}'. Only ${JSON.stringify(Object.values(APPOPS_ACTION))} actions are supported`);
    }
    if (typeof permissions === 'string' && permissions.toLowerCase() === ALL_PERMISSIONS_MAGIC) {
    ...
    const promises = (Array.isArray(permissions) ? permissions : [permissions]).map((permission) => this.adb.shell(['appops', 'set', appPackage, permission, action]));
