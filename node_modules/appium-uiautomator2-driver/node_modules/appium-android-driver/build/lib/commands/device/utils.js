...
export function prepareAvdArgs() {
    ...
    if (avdArgs) {
        if (Array.isArray(avdArgs)) {
            ...
            result.push(...util.shellParse(`${avdArgs}`).filter((entry) => typeof entry === 'string'));
        }
    }
    if (networkSpeed) {
...
export async function prepareEmulator(adb) {
    ...
    if (isEmulatorRunning) {
        if ((await prepareEmulatorForImageInjection.bind(this)(adb.sdkRoot)) || args.includes('-wipe-data')) {
...
export async function pushSettingsApp(throwIfError) {
    ...
    try {
        ...
        await retryInterval(HELPER_APP_INSTALL_RETRIES, HELPER_APP_INSTALL_RETRY_DELAY_MS, async () => await this.adb.installOrUpgrade(SETTINGS_APK_PATH, SETTINGS_HELPER_ID, {
        ...
        this.log.warn(`Ignored error while installing '${SETTINGS_APK_PATH}': ` +
