 ...
 */
export function parseWebviewNames(webviewsMapping, options = {}) {
 ...
 */
export async function getWebViewsMapping(opts = {}) {
...
export async function setupNewChromedriver(opts, curDeviceId, context) {
    ...
    for (const opt of Object.keys(opts)) {
        if (opt.endsWith(':chromeOptions')) {
            ...
            merge(opts.chromeOptions, opts[opt]);
        }
    }
    ...
    for (const opt of Object.keys(opts)) {
        if (opt.endsWith(':loggingPrefs')) {
            ...
            merge(opts.chromeLoggingPrefs, opts[opt]);
        }
    }
    ...
    if (opts.chromedriverGrantPermissions) {
        ...
        if (!chromePkg) {
            throw new Error(`'chromedriverGrantPermissions' is enabled but the Chrome package name could not be ` +
...
function createChromedriverCaps(opts, deviceId, webViewDetails) {
    ...
    const androidPackage = opts.chromeOptions?.androidPackage || opts.appPackage || webViewDetails?.info?.['Android-Package'];
    if (androidPackage) {
    ...
    if (KNOWN_CHROME_PACKAGE_NAMES?.includes(caps.chromeOptions.androidPackage) || isChrome) {
...
async function getPotentialWebviewProcs() {
    ...
    if (util.isEmpty(names)) {
        ...
        this.log.debug(`Parsed ${names.length} active devtools ${util.pluralize('socket', names.length, false)}: ` +
...
function isCompatibleCdpHost(host) {
    return (['localhost', 'localhost.localdomain'].includes(host) || host.endsWith('.localhost') || Boolean(net.isIP(host)));
}
