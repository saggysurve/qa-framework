...
import { CHROMIUM_WIN, KNOWN_CHROME_PACKAGE_NAMES, NATIVE_WIN, WEBVIEW_BASE, WEBVIEW_WIN, dismissChromeWelcome, getWebViewsMapping, parseWebviewNames, setupExistingChromedriver, setupNewChromedriver, shouldDismissChromeWelcome, } from './helpers.js';
...
export async function startChromedriverProxy(context, webviewsMapping) {
    ...
    if (this.sessionChromedrivers[context]) {
            ...
            if (!opts.extractChromeAndroidPackageFromContextName) {
                if (this.opts.enableWebviewDetailsCollection !== undefined && !this.opts.enableWebviewDetailsCollection) {
                    ...
                    for (const knownPackage of KNOWN_CHROME_PACKAGE_NAMES) {
                        ...
                        if ([APP_STATE.RUNNING_IN_BACKGROUND, APP_STATE.RUNNING_IN_FOREGROUND].includes(appState)) {
        ...
        cd = await setupNewChromedriver.bind(this)(opts, this.adb.curDeviceId, context);
...
export async function notifyBiDiContextChange() {
    ...
    this.eventEmitter.emit(BIDI_EVENT_NAME, makeContextUpdatedEvent(String(this.opts.automationName)?.toLowerCase(), name));
...
export async function startChromeSession() {
    ...
    if (knownPackages?.includes(this.opts.appPackage)) {
        opts.chromeBundleId = this.opts.appPackage;
    }
    else {
        opts.chromeAndroidActivity = this.opts.appActivity;
    }
    const chromedriver = await setupNewChromedriver.bind(this)(opts, this.adb.curDeviceId);
    ...
    chromedriver.on(Chromedriver.EVENT_CHANGED, async (msg) => {
        if (msg.state === Chromedriver.STATE_STOPPED) {
            try {
                ...
                this.log.warn(`Error handling chromedriver stop event for context ${CHROMIUM_WIN}: ${err.message}`);
            }
        }
    });
