...
export async function mobilePerformStatusBarCommand(command, component) {
    ...
    if (!action) {
        throw new errors.InvalidArgumentError(`The '${command}' status bar command is unknown. Only the following commands ` +
...
export function parseWindows(lines) {
    ...
    for (const [window, namePrefix] of unmatchedWindows) {
        ...
        result[window] = structuredClone(DEFAULT_WINDOW_PROPERTIES);
    }
    return result;
}
