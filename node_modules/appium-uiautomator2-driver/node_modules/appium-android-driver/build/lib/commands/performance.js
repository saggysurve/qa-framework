...
export const CPU_KEYS = ['user', 'kernel'];
...
export async function getPerformanceDataTypes() {
    return Object.keys(SUPPORTED_PERFORMANCE_DATA_TYPES);
}
/**
...
export async function getCPUInfo(packageName, retries = 2) {
    ...
    return await retryInterval(retries, RETRY_PAUSE_MS, async () => {
        ...
        const usagesPattern = new RegExp(`^.+\\/${util.escapeRegExp(packageName)}:\\D+([\\d.]+)%\\s+user\\s+\\+\\s+([\\d.]+)%\\s+kernel`, 'm');
