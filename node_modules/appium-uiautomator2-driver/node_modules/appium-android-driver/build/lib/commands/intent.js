 ...
 */
export async function startActivity(appPackage, appActivity, appWaitPackage, appWaitActivity, intentAction, intentCategory, intentFlags, optionalIntentArguments, dontStopAppOnReset) {
 ...
 */
export async function mobileStartActivity(wait, stop, windowingMode, activityType, display, user, intent, action, pkg, uri, mimeType, identifier, component, categories, extras, flags) {
 ...
 * @returns Promise that resolves to the command output string.
 */
export async function mobileBroadcast(receiverPermission, allowBackgroundActivityStarts, user, intent, action, pkg, uri, mimeType, identifier, component, categories, extras, flags) {
 ...
 * @returns Promise that resolves to the command output string.
 */
export async function mobileStartService(foreground, user, intent, action, pkg, uri, mimeType, identifier, component, categories, extras, flags) {
    const cmd = ['am'];
 ...
 */
export async function mobileStopService(user, intent, action, pkg, uri, mimeType, identifier, component, categories, extras, flags) {
    ...
    if (extras) {
        ...
        for (const item of extras) {
            ...
            if (!SUPPORTED_EXTRA_TYPES.includes(type)) {
                throw new errors.InvalidArgumentError(`Extra argument type '${type}' is not known. ` +
                    `Supported intent argument types are: ${SUPPORTED_EXTRA_TYPES.join(', ')}`);
            }
            if (util.isEmpty(key) || (typeof key === 'string' && key.trim() === '')) {
                throw new errors.InvalidArgumentError(`Extra argument's key in '${JSON.stringify(item)}' must be a valid string identifier`);
            }
            if (type === NO_VALUE_ARG_TYPE) {
                resultArgs.push(`--e${type}`, key);
            }
            else if (value === undefined) {
                throw new errors.InvalidArgumentError(`Intent argument type '${type}' in '${JSON.stringify(item)}' requires a valid value to be provided`);
            }
            else {
