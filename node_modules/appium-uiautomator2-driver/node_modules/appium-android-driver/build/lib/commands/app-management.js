 ...
 *
 */
export async function mobileListApps(user) {
    const opts = {};
    if (util.hasValue(user)) {
        opts.user = `${user}`;
    }
...
export async function terminateApp(appId, options = {}) {
    ...
    const timeout = util.hasValue(options.timeout) && !Number.isNaN(options.timeout) ? parseInt(String(options.timeout), 10) : 500;
    if (timeout <= 0) {
    ...
    try {
        ...
        const currentPidsSet = new Set(currentPids);
        if (!util.isEmpty(currentPids) && pids.some((pid) => !currentPidsSet.has(pid))) {
            this.log.warn(`Some of processes belonging to the '${appId}' application are still running ` +
 ...
 * `false` by default.
 */
export async function mobileInstallApp(appPath, checkVersion, timeout, allowTestPackages, useSdcard, grantPermissions, replace, noIncremental) {
    const opts = {
        timeout,
        allowTestPackages,
        useSdcard,
